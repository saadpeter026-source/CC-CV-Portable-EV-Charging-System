...
If we know the EV battery capacity is 309 Ah, the charging current is 35 A, and in our case we have only evaluated 10.8212 seconds of real charging time, we can calculate it like this. We know that a charging current of 35 A delivers 35 Ah of charge in exactly one hour. However, we do not have 1 hour of simulation; we only have 10.8212 seconds of simulation. We can convert 10.8212 seconds into hours. Since 3600 seconds is exactly one hour, we can apply this to the following equation: $$Q = I \times t$$. Since we know how to calculate time now, and we have the current as 35 A, we can calculate the charge: $$Q = 35 \times \frac{10.8212}{3600}$$, which gives us exactly 0.1052 Ah. We can use that value (0.1052 Ah) to find SOC %. We know that the EV battery capacity is 309 Ah, so dividing 0.1052 Ah by 309 Ah and multiplying it by 100 (SOC formula), we get exactly 0.0334%, which is the same percentage that came from the plot above.

One thing to keep in mind is that from 10.8212 to 11 seconds we can see the charging process came to a stop and remained constant for the rest of the experiment. This is due to the stop charging process of the current termination logic we have implemented. Another thing to account for is that we can barely see any change at approximately 8.6541, where the CC-CV transition occurs. Usually, in a CC-CV charging strategy, there is a slight bend that causes the CV process to charge a bit less than the CC process. The reason for this is that less current is being supplied as it is decreasing, so the SOC curve should be gaining less charge as time goes on. We can verify this by looking at the plot. For example, from 0 to 8.6541 the change in SOC is approximately 0.0272%, and from 8.6541 to 10.8212 seconds there is only a change of around 0.00628%. Although the CV region only lasts approximately 2.17 seconds, the SOC increase is only 0.00628%, compared to 0.0272% during the 8.6541-second CC region. This is expected because the charging current begins to decrease during CV mode, reducing the rate at which the battery gains charge.
