...
Although the voltage and current plots demonstrate successful implementation of the CC-CV charging strategy, the SOC % plot is also another validation tool we can use to estimate if the charging process is working. As mentioned in the description (SOC subsystem), we have started this experiment at **5% SOC** to demonstrate the charging process when the EV vehicle has run out of charge. Looking at the plot, we can see a 0.0334% increase in approximately 10.8212 seconds of simulation. This is great, but how can we validate that this system is actually working? Well, let's do a bit of math.

If we know the EV battery capacity is 309 Ah, the charging current is 35 A, and in our case we have only evaluated 10.5 seconds of real charging time, we can calculate it like this. We know that a charging current of 35 A delivers 35 Ah of charge in exactly one hour. However, we do not have 1 hour of simulation; we only have 10.5 seconds of simulation. We can convert 10.8212 seconds into hours. Since 3600 seconds is exactly one hour, we can apply this to the following equation: $$Q = I \times t$$. Since we know how to claucte time now, adn we have current as 35A, we can clacute the charge $$Q = 35 \times \frac{10.8212}{3600}$$ which gives us exaclay 0.1052Ah. we can use that value (0.1052Ah) to find SOC %. We know that the Ev battery cpacity is 309Ah so divingin 0.1052Ah and 309 and multiplg it by 100 (SOC fomula) we get excaly 0.0334% which is the same pernatage that came form the plot above. 

One thing to keep in mind is that from 10.8212 to 11 seconds we can see the chagring procress came to stop, and rmeained cotannt for the rest of the epxicment. This is due to the sotp chagring procress of the curent temrnaition logic we have implament. Another thing to acocutn for is that we cbarlyey see any chaneg at approxcnaly 8.6541 where the CC-CV trnastion occers. Usually in a CC-CV chagrign startgy there is a slight bend that cuases the CV procress to charge a bit less than the CC procress. the reason for this is that current is being suppied less as its decreasing so SOC curve should be gaianing less chc=arge as time goes on. We can verifty this by lookign at the plot. For exmaple from 0 to 8.6541 the chnage in SOC is approx 0.0272%, and from 8.6541 to 10.8212 seconds there is only a chnage of aorund 0.00628. "Although the CV region only lasts approximately 2.17 seconds, the SOC increase is only 0.00628%, compared to 0.0272% during the 8.6541-second CC region. This is expected because the charging current begins to decrease during CV mode, reducing the rate at which the battery gains charge.
