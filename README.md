...
Now that we know a little about what the **External Portable Charger** does, let's analyze the plant of this control system design. The EV battery we have parameterized is an approximated model of the **Cadillac Lyriq 2024** (all of the data can be found in the **data/** directory of this project). Since we could not model a much more detailed battery pack, which is typically represented using individual cells connected in series and parallel with specific dimensions, we constructed a much simpler Simscape model to reduce simulation time and maintain simplicity.

The EV battery has a nominal voltage of **350 V** without any initial conditions. When the initial condition is set to **5% SOC**, the voltage decreases to approximately **294.8 V**. The battery capacity is modeled as **309 Ah**, providing approximately **108.15 kWh** of energy when fully charged.

Since this experiment begins at **5% SOC**, the EV battery initially stores approximately **5.41 kWh** of energy. Therefore, it requires approximately **102.74 kWh** of additional energy to reach **100% SOC** (fully charged).

Now comes the interesting part of this EV model. Since the portable charger stores approximately **2.924 kWh** of energy, starting the system at **5% SOC** means it would take approximately **17 minutes** for the portable charger to become fully depleted. This is determined using the maximum charging power,

$$
P = VI = (294.8~\mathrm{V})(35~\mathrm{A}) \approx 10.3~\mathrm{kW}
$$

Using this power, the discharge time of the portable charger is

$$
t = \frac{E}{P}
= \frac{2.924~\mathrm{kWh}}{10.3~\mathrm{kW}}
\approx 0.284~\mathrm{hours}
\approx 17~\mathrm{minutes}
$$

This calculation assumes an ideal system with **100% efficiency**.

Thus, the EV battery would increase from **5% SOC** to approximately **7.7% SOC**, since

$$
\Delta \mathrm{SOC}
=
\frac{2.924~\mathrm{kWh}}
{108.15~\mathrm{kWh}}
\times 100\%
\approx 2.7\%
$$
...
$$
\mathrm{SOC}_{\mathrm{final}}
=
5\% + 2.7\%
\approx 7.7\%
$$

This analysis is important because it allows us to estimate how much driving range the external portable charger can provide before its energy is completely depleted. Since the **Cadillac Lyriq** has an estimated energy consumption of approximately **300 Wh/mile**, the additional driving range can be calculated as
...
\frac{2924~\mathrm{Wh}}
{300~\mathrm{Wh/mile}}
\approx 9.7~\mathrm{miles}
$$

Therefore, the external portable charger can provide approximately **9.7 miles** of additional driving range before its stored energy is exhausted.
