...
In this systme, for us to be able to verify if the chagring procress is wokring, we have to consider a faily simple mahtamical model caleld State of charge (SOC) using the method of columb counting. SOC in gerenal represnets the the amount of charge in a battery relative to its starting capcity. For insance in out mode speciaclly 309Ah represnts the full cpacity of the battery; howwver sine we want to simualte this in 5% SOC startung, we take 0.05 x 309 = 15.45Ahr. 15.45Ahr here would rperesnet the intial charge of our EV battery to repreent 5% SOC. To rperesent this subsystem in a more mathnacial way we can consider the equation obtained from the mathwroks documetnon $$\mathrm{SOC}(t)=\mathrm{SOC}(t_0)+\frac{1}{C_{\mathrm{rated}}}\int_{t_0}^{t} I_{\mathrm{batt}}\,dt$$. There are three improant things we have to carfually anyalzir in this eqaution: the numerator which represnts the chnage in the battery charge over time ($$\int_{t_0}^{t} I_{\mathrm{batt}}\,dt$$), and the dmeoanater represents rated battery cpacity ($$C_{\mathrm{rated}}$$), and $\mathrm{SOC}(t_0)$ is the initial state of charge at the initial time $t_0$. 

Although this equation describes the general formulation of the Coulomb Counting method, our implementation is simplified by utilizing the remaining charge output provided by the Simscape Battery model instead of explicitly integrating the battery current. this is done by considerd ng this fomrula: $$\mathrm{SOC}=\frac{Q_{\mathrm{remaining}}}{3600\,C_{\mathrm{rated}}}$$
